function [model] = test_stvm_train(trainX,trainY,testX,options)
...
if sizeC == 2
        ...
        if size(trainX,1) < size(testX,1)
            diff = size(trainX,1) - size(testX,1);
            sampleSize = round(diff / sizeC)
            for c = C
                idxs = find(trainY == c);
                classData= trainX(idxs,:);
...
%     trainX = U*SZ*V';
    ...
    theta = options.theta;
    Ks = kernel(options.ker, trainX', [],theta);
    Kt = kernel(options.ker, testX', [],theta); 
    
    [US,SZ,VS] = svd(Ks,'econ');
    [U,S,V] = svd(Kt,'econ');
    Kl = U*SZ*V';
